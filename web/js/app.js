...
var App = function () {
    ...
    return {
        ...
        blockUI: function (el) {
            jQuery(el).block({
                message: '<img src="/web/assets/img/loading.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
        ...
        // Ajax support 
        jsonRequest: function(func, input, reloadOnError) {
            if ( typeof(reloadOnError) === 'undefined' ) reloadOnError = false;

            App.blockUI(jQuery(".widget-body"));
            $.ajax({
                type:'POST', dataType:"json", data:input,
                success: function(response) {
                    func(response)
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    alert("textStatus: "+textStatus+"\nerrorThrown: "+errorThrown);
                    if( reloadOnError ) window.location.reload();
                },
